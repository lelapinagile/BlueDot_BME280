...
uint8_t BlueDot_BME280::init()
{
	...
	//#########################
	switch (parameter.communication)
	{
	case DeviceParameter::CommunicationMode::SPI_SOFT: //Software SPI Communication
	{
	...
	case DeviceParameter::CommunicationMode::SPI_HARD: //Hardware SPI Communication
	{
		digitalWrite(parameter.SPI_cs, HIGH); //Chip Select Pin to HIGH
		pinMode(parameter.SPI_cs, OUTPUT); //Chip Select Pin as Output
...
//##########################################################################
void BlueDot_BME280::readCoefficients()
{
...
//##########################################################################
void BlueDot_BME280::writeIIRFilter()
{
	...
	const byte value = (static_cast<uint8_t>(parameter.IIRfilter) << 2) & 0b00011100;
	writeByte(BME280_CONFIG, value);
}
//##########################################################################
void BlueDot_BME280::writeCTRLMeas()
{
...
float BlueDot_BME280::readPressure(void)
{
	if (parameter.pressOversampling != DeviceParameter::Filter::Off) //disabling the pressure measurement function
	{
		...
		int32_t adc_P = (uint32_t)readByte(BME280_PRESSURE_MSB) << 12;
		adc_P |= (uint32_t)readByte(BME280_PRESSURE_LSB) << 4;
		adc_P |= (readByte(BME280_PRESSURE_XLSB) >> 4) & 0b00001111;

		int64_t var1 = ((int64_t)t_fine) - 128000;
		int64_t var2 = var1 * var1 * (int64_t)bme280_coefficients.dig_P6;
...
float BlueDot_BME280::convertTempKelvin()
{
	...
	if (parameter.tempOutsideCelsius != 999)
	{
	...
	{
		if (parameter.tempOutsideFahrenheit == 999)
		{
...
//##########################################################################
float BlueDot_BME280::readAltitudeFeet()
{
	return readAltitudeMeter() / 0.3048;
}

//##########################################################################
float BlueDot_BME280::readAltitudeMeter()
{
	const float tempOutsideKelvin = convertTempKelvin();

	float heightOutput = readPressure();
...
float BlueDot_BME280::readHumidity()
{
	if (parameter.humidOversampling != DeviceParameter::Filter::Off) //disabling the humitidy measurement function
	{
		...
		var1 = (((((adc_H << 14) - (((int32_t)bme280_coefficients.dig_H4) << 20) - (((int32_t)bme280_coefficients.dig_H5) * var1)) +
				  ((int32_t)16384)) >>
				 15) *
				(((((((var1 * ((int32_t)bme280_coefficients.dig_H6)) >> 10) * (((var1 * ((int32_t)bme280_coefficients.dig_H3)) >> 11) + ((int32_t)32768))) >> 10) + ((int32_t)2097152)) *
					  ((int32_t)bme280_coefficients.dig_H2) +
				  8192) >>
				 14));
...
float BlueDot_BME280::readTempC(void)
{
	if (parameter.tempOversampling != DeviceParameter::Filter::Off) //disabling the temperature measurement function
	{
		...
		const int64_t var1 = ((((adc_T >> 3) - ((int32_t)bme280_coefficients.dig_T1 << 1))) * ((int32_t)bme280_coefficients.dig_T2)) >> 11;
		const int64_t var2 = (((((adc_T >> 4) - ((int32_t)bme280_coefficients.dig_T1)) * ((adc_T >> 4) - ((int32_t)bme280_coefficients.dig_T1))) >> 12) *
							  ((int32_t)bme280_coefficients.dig_T3)) >>
							 14;
		t_fine = var1 + var2;
...
void BlueDot_BME280::writeByte(byte reg, byte value)
{
	switch (parameter.communication)
	{
	case DeviceParameter::CommunicationMode::SPI_SOFT:
	{
	...
	case DeviceParameter::CommunicationMode::SPI_HARD:
	{
		digitalWrite(parameter.SPI_cs, LOW);
